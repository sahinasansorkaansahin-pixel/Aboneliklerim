...
keys_to_update = [
    "premium_title", "premium_subtitle", "premium_get_service",
    "plan_monthly", "plan_yearly", "plan_lifetime", "choose_plan",
    "premium_limit_reached", "premium_limit_max"
]
...
for val_dir in targets:
    ...
    try:
        ...
        existing_keys = [c.get('name') for c in t_root if c.tag == 'string']
        
        updated = False
        for key in keys_to_update:
            if key not in existing_keys:
                try:
                    translated = translator.translate(eng_texts[key])
                    translated = translated.replace("'", "\\'")
                    new_element = ET.Element("string", name=key)
                    new_element.text = translated
                    t_root.append(new_element)
                    updated = True
                except:
                    pass
            else:
        ...
        if updated:
            ...
            print(f"Added premium strings to {val_dir}")
    except:
        pass
